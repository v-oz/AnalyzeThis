...
buildFilters = function(filters, filter) {
	...
	return filters;
		...
		CSVparse= function(content, clear = true){
			var f, author = {}, csvObj = Papa.parse(content,{delimiter:";", header:true, fastMode:false});
			if (clear) this.om.removeAll();
			...
			// ymaps.modules.require(['DatasetOps'],function ( DatasetOps) {
				// var ds = new DatasetOps(mhMap);
...
ymaps.modules.define('DatasetOps', ['util.defineClass', 'plugin.AuthorListBoxControl'],
function (provide, defineClass, AuthorListBoxControl) {
	function DatasetOps(props) {
		this.map = props.map;
		this.om = props.om;
		var obj = {r:new FileReader(), dsSource:""}, st = {m:this.map, om:};
		document.getElementById("dataset").addEventListener("change", function(obj) {
			return function() {			
				var file, fileList = this.files;
				if(fileList.length == 1){
					file = fileList.item(0);
					if (file.type == "application\/json") {	obj.dsSource = "JSON"; } else {obj.dsSource = "CSV";}
					obj.r.readAsText(file);
				}
			} 
		}(obj), false);
		obj.r.addEventListener("loadend", function(obj) {
			return function() { 
				if (obj.dsSource == "JSON") { /* dataObj = JSON.parse(reader.result); fillPlacemarks(dataObj, false); */}
				else {CSVparse(obj.r.result, false);}
				boundMap(mhMap, objectManager.getBounds());
			}
		}(obj), false);
	}
	defineClass(DatasetOps, {
		updateAuthorsList: function(props) {
			this.map.controls.remove(props.list);
			props.list = new AuthorListBoxControl(props.author);
			this.map.controls.add(props.list, { float: 'left', floatIndex: 1});
		},
		setFilter2Monitor: function(props){
			props.fm.add('filters', function(filters) {
				filters = ymaps.util.extend({}, filters, 
					props.listBCtype.state.get('filters'), 
					props.listBCstatus.state.get('filters'), 
					props.listBCauthor.state.get('filters'), 
					props.listBCfixage.state.get('filters'));
				props.om.setFilter(this.getFilterFunction(filters, props.author));
			},this);
		},
		getFilterFunction: function(filters, author) {
			return function(obj) {
				...
				return filters[a] && filters[t] && filters[s] //&& filters[fa] && f
			}
		},
		destroy: function () {}
	});
